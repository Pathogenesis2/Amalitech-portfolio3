...
function Container() {
    ...
    if (name.trim().length > 1) {
      if (continent !== "Filter by Region" && continent !== "All Countries") {
        if (
          ...
          setFilteredCountries(
            countries.filter((country) => country.region.includes(continent))
          );
        }
      } else {
        if (
  ...
  return (
    <div className={toggleMode ? "body-light" : "body-dark"}>
      <Routes>
        <Route
          path="/CountryName/:CountryInfo"
          Component={() => (
            <GetCountryInfo toggleMode={toggleMode} countries={countries} />
          )}
        />
        <Route
          path="/"
          Component={() => (
            <CountriesContainer
              ...
              setClickMonintor={setClickMonitor}
